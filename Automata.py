    ...
    def LoadAutomataFromCsv(self, path):
        ...
        with open(path, 'r', newline='') as f:
            ...
            for row in reader:
                ...
                alphabet = json.loads(row['alphabet'])
                succ_list = json.loads(row['successors_json'])
                succs = [ (tuple(s[0]), s[1]) for s in succ_list ]
                key = state
                Tmap.setdefault(key, {})[alphabet] = succs
        
        self.TransMap = Tmap

    def SaveAutomataToCsv(self, path):
        """Saves automata into a CSV file."""
        import csv, json
    
        with open(path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['state', 'alphabet', 'successors_json'])
            for (x, q), ctrl_dict in self.TransMap.items():
                for u, succs in ctrl_dict.items():
                    writer.writerow([json.dumps(x), json.dumps(u), json.dumps(ctrl_dict)])
